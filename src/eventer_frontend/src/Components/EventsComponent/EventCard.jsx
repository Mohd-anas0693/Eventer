...
const EventCard = ({ item }) => {
    ...
    status = nowTime < startTime ? "Upcoming" : nowTime > startTime && nowTime < endTime ? "Ongoing" : nowTime > startTime && nowTime > endTime ? "Past" : 'No Status';
        ...
        <div className="lg:w-2/6 sm:w-3/6 w-full p-4">
            <div className="w-full h-96 px-8 bg-white rounded-xl shadow-lg flex flex-col justify-between">
                <div className='flex justify-end mt-5  sm:mt-[5%] mr-[-5%]'>
                    ...
                    <div className={`shadow-xl px-2 text-white rounded-full uppercase border-[2px] ${status === 'Upcoming' ? `border-yellow-700 bg-yellow-700` : status === 'Ongoing' ? `border-green-700 bg-green-700` : status === 'Past' ? `border-red-700 bg-red-700` : 'border-black bg-black'}`}>
                        <p className='text-xs font-semibold'>{status ?? "Loading"}</p>
                    </div>
                </div>
