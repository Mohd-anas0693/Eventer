...
const AdminLogin = () => {
    ...
    const [adminHelpText, setAdminHelpText] = useState("Log in and give developer your principal id to make you an admin, If you already assigned as admin, just Login to continue.");
    ...
    useEffect(() => {
        ...
        if (isAdmin === true) {
            setAdminHelpText(`Welcome Admin, Redirecting to Dashboard...`);
            navigate("/dashboard");
        } else if (isAdmin === false) {
            setAdminHelpText(`Copy and Give developer your principal id to make you an admin, Please Reload if you get Anonymous Principal := ${identity.getPrincipal().toText()}`);
        }
    }, [isAdmin]);
            ...
            <div className="flex flex-col items-center justify-center w-full bg-[#f1f1f1]" style={{ height: `calc(100vh - 93px)` }}>
                <div className="w-full max-w-xs sm:max-w-sm md:max-w-md lg:max-w-lg xl:max-w-xl 2xl:max-w-2xl p-4 sm:p-6 md:p-10 lg:p-10 xl:p-12 2xl:p-14 bg-white rounded-xl shadow-lg flex flex-col gap-4">
                    ...
                    <div className="flex flex-col mt-4 w-full justify-center items-center pt-4">
                        ...
                        <p className="font-bold cursor-pointer text-sm text-blue-900 hover:text-blue-700 underline mt-4" onClick={() => window.location.href = "/login"}>Continue as attendee</p>
                    </div>
                </div>
            </div>
        </>
