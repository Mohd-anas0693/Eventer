    ...
    const validationSchema = yup.object(
        {
            ...
            start_date_time:
                filledValues?.status === statusPast || filledValues?.status === statusOngoing ?
                    yup.string() :
                    yup.string()
                        .test('is-non-empty', required, (value) => /\S/.test(value))
                        .required(required)
                        .test('is-future-date', mustBeFuture
                        , (value) => {
            ...
            end_date_time:
                filledValues?.status === statusPast ?
                    yup.string() :
                    yup.string()
                        .test('is-non-empty', required, (value) => /\S/.test(value))
                        .required(required)
        ...
        <div className="w-full lg:w-2/4 h-full flex items-start justify-start">
            <div className="bg-white px-4 py-7 rounded-xl w-full lg:w-[600px] mb-2 mt-2 my-16 shadow-lg ">
                <div className='flex justify-end'>
                    <div className={`shadow-xl px-2 text-white rounded-full uppercase border-[2px] ${filledValues?.status === statusUpcoming ? `border-yellow-700 bg-yellow-700` : filledValues?.status === statusOngoing ? `border-green-700 bg-green-700` : filledValues?.status === statusPast ? `border-red-700 bg-red-700` : 'border-black bg-black'}`}>
                        <p className='text-xs font-semibold'>{filledValues?.status ?? statusNoStatus}</p>
                    </div>
                </div>
                        ...
                        <Controller
                            ...
                            render={({ field }) => (
                                <input
                                    ...
                                    type="datetime-local"
                                    id={`start_date_time`}
                                    disabled={buttonDisabled || filledValues?.status === statusPast || filledValues?.status === statusOngoing}
                                    onChange={(e) => { field.onChange(e.target.value); setValue('start_date_time', e.target.value) }}
                                    className={`w-full p-2 rounded-md mt-1 ${(buttonDisabled || filledValues?.status === statusPast || filledValues?.status === statusOngoing) ? 'bg-gray-400' : ''}`} onKeyDown={handleKeyArrowDown}
                        ...
                        <Controller
                            ...
                            render={({ field }) => (
                                <input
                                    ...
                                    type="datetime-local"
                                    id={`end_date_time`}
                                    disabled={buttonDisabled || filledValues?.status === statusPast}
                                    onChange={(e) => { field.onChange(e.target.value); setValue('end_date_time', e.target.value) }}
                                    className={`w-full p-2 rounded-md mt-1 ${(buttonDisabled || filledValues?.status === statusPast) ? 'bg-gray-400' : ''}`} onKeyDown={handleKeyArrowDown}
